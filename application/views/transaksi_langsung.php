...
<body>
                            ...
                            <div class="payment-summary">
                                ...
                                <div class="payment-summary-item">
                                    <div class="payment-summary-name">
                                        <?= implode(' ', array_slice(explode(' ', $konten->judul), 0, 5)) ?>
                                    </div>
                                    <div class="payment-summary-price">Rp<?= number_format($konten->harga, 0, ',', '.') ?></div>
                                </div>
                ...
                <div class="payment-right">
                    ...
                    <form action="<?= base_url('transaksi/bayar/') ?>" method="post" class="payment-form" enctype="multipart/form-data">
                        ...
                        <div class="payment-method">
                            ...
                            <input type="radio" name="via" value="Dana" id="method-3">
                            <label for="method-3" class="payment-method-item">
                                <img src="<?= base_url('transaksi/') ?>images/dana.png" alt="">
                            </label>

                            <input type="radio" name="via" value="Ovo" id="method-4">
                            <label for="method-4" class="payment-method-item">
                                <img src="<?= base_url('transaksi/') ?>images/ovo.png" alt="">
                            </label>

                            <input type="hidden" name="total" value="<?= $konten->harga ?>">
                        </div>

                        <div id="barcode-container" style="display: none;">
                            <img src="<?= base_url('transaksi/images/barcode.jpg') ?>" width="400">
                        </div>
                        ...
                        <input type="hidden" name="belanja" value="<?= $konten->judul ?>">
    ...
    <!-- end: Payment -->
                    <script>
                        // Get radio buttons and barcode container
                        const barcodeContainer = document.getElementById('barcode-container');
                        const radioButtons = document.querySelectorAll('input[name="via"]');

                        // Add event listener to check the payment method
                        radioButtons.forEach(button => {
                            button.addEventListener('change', function() {
                                if (this.value === 'Dana' || this.value === 'Gopay') {
                                    barcodeContainer.style.display = 'block'; // Show barcode for Dana or Gopay
                                } else {
                                    barcodeContainer.style.display = 'none'; // Hide barcode for other methods
                                }
                            });
                        });

                        if (document.querySelector('input[name="via"]:checked').value === 'Dana' || document.querySelector('input[name="via"]:checked').value === 'Gopay') {
